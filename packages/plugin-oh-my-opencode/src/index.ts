...
import {
  ...
  createUnifiedFlowHook,
} from "./hooks";
import { createUserInteractionDetectorHook } from "./hooks/user-interaction-detector";
import {
  contextCollector,
...
const OhMyOpenCodePlugin: Plugin = async (ctx) => {
  ...
  const unifiedFlow = isHookEnabled("unified-flow")
    ? createUnifiedFlowHook(ctx)
    : null;

  const userInteractionDetector = createUserInteractionDetectorHook(ctx);
  ...
  const verification = createVerificationTool(ctx);
  const userInteraction = createUserInteractionTool(ctx);
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      await userInteractionDetector["tool.execute.after"](input, output);
