...
检查Antigravity模型是否在模型列表中：

**English**: Check if Antigravity models are in the model list:
...
**症状 / Symptom**: `opencode --list-models` 中看不到Antigravity模型

**解决方案 / Solution**:
1. 检查Provider配置是否正确
2. 确认模型ID格式正确（`antigravity-gemini-3-flash`）
3. 检查插件是否正确加载
...
```bash
# 运行多个任务，观察是否自动切换账号 / Run multiple tasks, observe if accounts switch automatically
opencode run -m google/antigravity-gemini-3-pro-high -p "Test message 1"
opencode run -m google/antigravity-gemini-3-pro-high -p "Test message 2"
```
...
**症状 / Symptom**: 模型列表中看不到Antigravity模型  
**English**: Antigravity models not visible in model list
...
| OpenCode版本 | oh-my-opencode | opencode-antigravity-auth | 状态 |
|-------------|----------------|---------------------------|------|
| ≥ 1.0.150 | ✅ 支持 | ✅ 支持 | ✅ 推荐 |
| 1.0.100 - 1.0.149 | ⚠️ 部分支持 | ⚠️ 部分支持 | ⚠️ 可能有问题 |
| < 1.0.100 | ❌ 不支持 | ❌ 不支持 | ❌ 不兼容 |
...
1. **最小化配置模板** / **Minimal Configuration Template**
   - 文件位置: `docs/Oh_My_OpenCode_配置模板_最小化_v1.0_20260125_AI.json`
   - 只包含必需配置项，适合快速测试
   - **English**: Contains only essential items, suitable for quick testing

2. **完整配置模板** / **Complete Configuration Template**
   - 文件位置: `docs/Oh_My_OpenCode_配置模板_完整版_v1.0_20260125_AI.json`
   - 包含所有可选配置项，适合生产使用
   - **English**: Contains all optional items, suitable for production use

**使用方法 / Usage**:

```bash
# 复制模板到配置目录 / Copy template to config directory
cp docs/Oh_My_OpenCode_配置模板_最小化_v1.0_20260125_AI.json ~/.config/opencode/opencode.json

# 或使用完整版 / Or use complete version
cp docs/Oh_My_OpenCode_配置模板_完整版_v1.0_20260125_AI.json ~/.config/opencode/opencode.json
...
```json
"plugin": ["opencode-antigravity-auth@1.2.9"]
```
然后重启OpenCode。
...
- 初始版本
- 包含完整的安装、配置和使用指南
- 涵盖oh-my-opencode和opencode-antigravity-auth的集成
- 提供详细的故障排查和最佳实践
